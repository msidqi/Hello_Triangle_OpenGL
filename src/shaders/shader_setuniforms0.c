...
/*   Updated: 2020/10/24 17:23:10 by msidqi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	glUniform2fv(glGetUniformLocation(this->program_id, name), 1,
													(const float *)&vec[0]);
}
...
{
	glUniform3fv(glGetUniformLocation(this->program_id, name), 1,
													(const float *)&vec[0]);
}

void	set_vec4(struct s_shader *this, const char *name, const t_vec4f *vec)
{
	glUniform4fv(glGetUniformLocation(this->program_id, name), 1,
													(const float *)&vec[0]);
}
